...
def get_data(conditions, filters):
    ...
    # Group popup rows per sales order, keeping them in the same order
    # they were fetched (soi.idx ASC), which matches the Sales Order
    # Item table row sequence.
    for row in data:
    ...
    # Safety net: explicitly sort each sales order's popup rows by row_idx,
    # in case any downstream grouping/dedup logic ever changes list order.
    for so in so_popup:
        so_popup[so] = sorted(so_popup[so], key=lambda r: r["row_idx"])
