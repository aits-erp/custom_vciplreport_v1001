...
@frappe.whitelist()
def get_invoice_details(customer, from_date, to_date, sales_person=None):
    """
    Fetch invoice details for the clicked amount
    """
    try:
        ...
        # If sales person filter is applied, join with Sales Team table
        if sales_person:
            query += """
                AND EXISTS (
                    SELECT 1 
        ...
        frappe.log_error(frappe.get_traceback(), "Sales Person Report - Invoice Details")
        return {"error": str(e)}
