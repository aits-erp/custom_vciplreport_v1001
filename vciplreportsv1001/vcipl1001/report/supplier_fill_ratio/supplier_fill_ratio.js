frappe.query_reports["Supplier Fill Ratio"] = {
    ...
    formatter(value, row, column, data, default_formatter) {
        ...
        if (column.fieldname === "order_count" && data.order_popup) {
            ...
            value = `<a style="font-weight:bold;color:#1674E0;cursor:pointer"
                onclick='show_po("${safe}", "${data.party}")'>
                ${data.order_count}
            </a>`;
        }
...
window.show_po = function(encoded, party) {

    let rows = JSON.parse(decodeURIComponent(encoded));

    let tot_qty = 0, tot_rec = 0;

    let html = `<div id="print_po">
    <h3>Pending Purchase Orders — ${party}</h3>
    ...
    let total_ratio = tot_qty ? ((tot_rec / tot_qty) * 100).toFixed(2) : 0;
    ...
    d.fields_dict.html.$wrapper.html(`
        ...
        <button class="btn btn-primary"
            onclick="print_section('print_po')">Print</button>
    `);
...
window.show_items = function(encoded) {
    ...
    let tot_qty = 0, tot_rec = 0, tot_pen = 0;
    ...
    rows.forEach(r => {
        ...
        let ratio = r.qty ? ((r.received / r.qty) * 100).toFixed(2) : 0;
