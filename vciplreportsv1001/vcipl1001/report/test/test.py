...
def format_report_data(filters: Filters, item_details: dict, to_date: str) -> list[dict]:
	...
	for _item, item_dict in item_details.items():
		...
		# Add Batch No column
		batch_no = getattr(details, 'batch_no', '')
		row.append(batch_no)
...
class FIFOSlots:
	...
	def __update_balances(self, row: dict, key: tuple | str):
		...
		# Store batch_no in details
		if hasattr(row, 'batch_no'):
			self.item_details[key]["details"].batch_no = row.batch_no
	...
	def __aggregate_details_by_item(self, wh_wise_data: dict) -> dict:
		...
		for key, row in wh_wise_data.items():
			...
			# Aggregate batch_no (if multiple, take first or combine)
			if row["details"].get("batch_no"):
				item_row["details"].batch_no = row["details"].batch_no
